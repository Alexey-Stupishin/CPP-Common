...
static double factorials[] = {1.0f, 1.0f, 2.0f, 6.0f, 24.0f, 120.0f, 720.0f, 5040.0f, 40320.0f, 362880.0f, 3628800.0f, 39916800.0f, 479001600.0f, 6227020800.0f, 87178291200.0f,
...
static double CnkS_123[] = {0.0f, 1.3333333e-01f, 7.6190476e-02f, 5.0793651e-02f, 3.6940837e-02f, 2.8416028e-02f, 2.2732823e-02f, 1.8721148e-02f,
...
static double CnkS_121[] = {0.0, 6.6666667e-01f, 5.3333333e-01f, 4.5714286e-01f, 4.0634921e-01f, 3.6940837e-01f, 3.4099234e-01f, 3.1825952e-01f,
...
static double factorials[] = {1.0, 1.0, 2.0, 6.0, 24.0, 120.0, 720.0, 5040.0, 40320.0, 362880.0, 3628800.0, 39916800.0, 479001600.0, 6227020800.0, 87178291200.0,
...
static double CnkS_123[] = {0.0, 1.3333333e-01, 7.6190476e-02, 5.0793651e-02, 3.6940837e-02, 2.8416028e-02, 2.2732823e-02, 1.8721148e-02,
...
static double CnkS_121[] = {0.0, 6.6666667e-01, 5.3333333e-01, 4.5714286e-01, 4.0634921e-01, 3.6940837e-01, 3.4099234e-01, 3.1825952e-01,
...
inline double v_erfc(double _x)
{
...
// abs.error <= 3e-7
    ...
    double x = fabs(_x);
#ifdef AS_REAL_AS_FLOAT
	double v = (((((0.0000430638f*x + 0.0002765672f)*x + 0.0001520143f)*x + 0.0092705272f)*x
				+ 0.0422820123f)*x + 0.0705230784f)*x + 1.0f;
#else
	double v = (((((0.0000430638*x + 0.0002765672)*x + 0.0001520143)*x + 0.0092705272)*x
...
inline double v_gamma(double _x) // 1 <= x <= 33
{
    _x -= 1;
...
inline double v_getmax(double x1, double x2)
{
    return x1 > x2 ? x1 : x2;
}

inline double v_getmin(double x1, double x2)
{
    return x1 < x2 ? x1 : x2;
}

inline double v_dmax(int n, double *x, int *_im = nullptr)
{
    double m = -DBL_MAX;
    int im = -1;
    for (int i = 0; i < n; i++)
...
inline bool v_interpInArray(int n, double *x, double *Y, double x0, double *Y0)
...
inline bool v_interpInArrayEqdist(int n, double *x, double x0, double *Y0)
{   // suppose x is sorted ascending
    if (x0 < x[0])
        return false;
    if (x0 > x[n - 1])
