...
public:
    uint32_t Create(int *N, double *f, double *fy = nullptr, double *fz = nullptr);
    uint32_t Create(char *name, int dim, int *N, double *data);
    uint32_t Copy(double *f, double *fy = nullptr, double *fz = nullptr, int idx = 0);
    uint32_t Delete();
    int Write(FILE *fid);

    static int WriteHeader(FILE *fid);
    static size_t Write(FILE *fid, CbinData *);
    static size_t Write(FILE *fid, double *, size_t, char *);
