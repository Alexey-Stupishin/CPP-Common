...
//------------------------------------------------------------------
uint32_t CbinDataStruct::Copy(double *f, double *fy, double *fz, int idx)
{
    size_t sz = sizeof(double)*data[idx].N[0]*data[idx].N[1]*data[idx].N[2];
...
//------------------------------------------------------------------
uint32_t CbinDataStruct::Create(char *_name, int dim, int *N, double *_data)
{
    Delete();

    nRead = 1;

    strcpy(data[0].name, _name);
    data[0].nDims = dim;
    ...
    sz *= sizeof(double);
    data[0].data = new uint8_t[sz];
    data[0].type = MFO_TYPE_DOUBLE;

    memcpy(data[0].data, _data, sz);

    return 0;
}

//------------------------------------------------------------------
uint32_t CbinDataStruct::Create(int *N, double *f, double *fy, double *fz)
{
    Delete();

    uint64_t sz = sizeof(double)*N[0]*N[1]*N[2];
...
//------------------------------------------------------------------
size_t CbinDataStruct::Write(FILE * fid, double *d, size_t lng, char * name)
{
    CbinData data;
